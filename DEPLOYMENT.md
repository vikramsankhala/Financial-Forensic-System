...
Render can run the backend via Docker.
...
3. **Set environment variables in Render:**
   ...
   - `JWT_SECRET`
   - `OPENAI_API_KEY` (if using demo audio)
   - Optional: `REDIS_URL`, `REDIS_ENABLED`, `NEO4J_*`
4. **Deploy** and wait for build to finish.
5. **Run migrations + seed/training (one-time):**
   - Use Render Shell or connect to the service shell:
     ```bash
     alembic upgrade head
     python scripts/seed_data.py
     python scripts/train_model.py
...
### Backend Deployment (Koyeb - Free Tier)

Koyeb runs the backend as a Docker container from your repo.

1. **Create a new Service** in Koyeb and connect your GitHub repo.
2. **Service settings:**
   - Build: `Dockerfile`
   - Root Directory: `backend`
   - Instance type: Free
...
Use Netlify for the Next.js frontend and keep the backend on Fly.io.
...
3. **Set environment variables in Netlify:**
   ...
   - `OPENAI_API_KEY` = your OpenAI API key (needed for audio demos)
4. **Deploy:**
   - Click **Deploy site**
5. **Verify:**
   - Visit the Netlify URL
   - Log in and load dashboard data
...
### Local Env Templates (Non-secret)

Templates are provided in the repo to help you create local env files:
- Root: `env.template`
